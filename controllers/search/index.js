...
async function helperAction(ctx) {
  ...
  const keywords = await mysql('goods')
    .select('goods.*', 'cSessionInfo.user_info')
    .join('cSessionInfo', 'goods.openid', 'cSessionInfo.open_id')
    .where('title', 'like', `%${keyword}%`).orWhere('describe', 'like', `%${keyword}%`)
